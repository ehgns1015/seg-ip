...
/**
 * InventoryList component for displaying inventory items in a table.
 * Provides edit and delete functionality for each item.
 *
 * @component
 * @param {InventoryListProps} props - The component props
 * @returns {JSX.Element} The inventory list component
 */
const InventoryList: React.FC<InventoryListProps> = ({
  ...
  const handleEdit = (item: string) => {
    router.push(`/inventory/edit/${encodeURIComponent(item)}`);
  };
  ...
  const handleDelete = async (item: string) => {
    if (confirm(`Are you sure you want to delete "${item}"?`)) {
      try {
        setIsDeleting(item);
        await inventoryApiService.deleteItem(item);
        onItemDeleted();
      } catch (err) {
